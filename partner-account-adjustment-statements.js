/* RETRADE adjustment-aware partner statement exporters — v1.4.99 */
(function(){
  'use strict';
  ...
  function tables(s){
    ...
    var tx=[['Date','Type','Reference / reason','Gross (£)','Adjustment (£)','Cash payment / RETRADE impact (£)','Status']];
    ...
    (s.accountAdjustments||[]).forEach(function(a){tx.push([a.date,'Account adjustment',a.reason,a.amount,round(-a.amount),a.retradeImpact,'Partner payable reduced'+(a.applied?' · '+gbp(a.applied)+' applied':'')]);});
  ...
  async function csv(){
    try{var s=await statement(),tab=tables(s),out=[];tab.summary.forEach(function(r){out.push(r);});out.push([],['SOLD ITEMS']);tab.sold.forEach(function(r){out.push(r);});out.push([],['PAYMENTS, ACCOUNT ADJUSTMENTS & RETURNS']);tab.transactions.forEach(function(r){out.push(r);});download('\uFEFF'+csvRows(out),'text/csv;charset=utf-8','RETRADE_'+safe(s.account.name)+'_Statement_'+s.period.slug+'.csv');try{toast('Partner statement CSV downloaded');}catch(_){}}
  ...
  function writePdf(s,logo){
    ...
    var navy=theme&&theme.colors?theme.colors.navy:[12,20,36],gold=theme&&theme.colors?theme.colors.gold:[247,183,55],muted=theme&&theme.colors?theme.colors.muted:[110,118,130],line=theme&&theme.colors?theme.colors.line:[224,228,233],pale=[247,249,252],green=theme&&theme.colors?theme.colors.green:[46,125,50],L=14,R=196,W=182,y=0;
    function colour(fn,c){fn.apply(doc,c);}
    function brand(){
      if(theme&&typeof theme.brandBanner==='function'){theme.brandBanner(doc,logo);return;}
      colour(doc.setFillColor,navy);doc.roundedRect(L,12,W,25,1.6,1.6,'F');doc.setFont('helvetica','bold');doc.setFontSize(17);colour(doc.setTextColor,gold);doc.text('RETRADE',20,27);doc.setFontSize(7.2);doc.setTextColor(255,255,255);doc.text("THE RESELLER'S BACK POCKET",190,26.2,{align:'right'});
    }
    ...
    function row(label,value,bold){ensure(8);if(bold){colour(doc.setFillColor,pale);doc.roundedRect(L,y-4,W,7,1,1,'F');}doc.setFont('helvetica',bold?'bold':'normal');doc.setFontSize(8.3);colour(doc.setTextColor,bold?navy:[28,36,48]);doc.text(label,L+2,y);doc.text(value,R-2,y,{align:'right'});y+=7;rule();}

    brand();y=50;
    doc.setFont('helvetica','bold');doc.setFontSize(18.5);colour(doc.setTextColor,navy);doc.text('PARTNER STATEMENT',L,y);
    y+=8;doc.setFont('helvetica','bold');doc.setFontSize(13);doc.text(String(s.account.name||'Partner'),L,y);
    y+=6;doc.setFont('helvetica','normal');doc.setFontSize(8.5);colour(doc.setTextColor,muted);doc.text(s.period.label+' · Adjustments reconciled',L,y);y+=11;

    section('Account summary');
    row('Revenue',gbp(s.totals.revenue));
    row('Margin before partner/supplier payout',gbp(s.totals.preDistribution));
    row('Gross partner/supplier earned',gbp(s.totals.grossPartnerEarned));
    if(Math.abs(s.totals.partnerAdjustment)>0.009)row('Account adjustments','−'+gbp(Math.abs(s.totals.partnerAdjustment)));
    row('Net partner/supplier earned',gbp(s.totals.partnerEarned),true);
    ...
    y+=6;section('Sold items — final net position');
    if(!s.sales.length)row('No sold items in this period','');
    s.sales.forEach(function(r){
      var adj=Math.abs(r.accountAdjustment||0)>0.009,label=String(r.item||'Item')+(r.saleNo?' · '+r.saleNo:''),lines=doc.splitTextToSize(label,94),h=Math.max(adj?16:11,lines.length*3.5+(adj?10:6));ensure(h);
      ...
      doc.setFont('helvetica','normal');doc.text('Partner '+gbp(r.partnerAmount),R-31,y+3,{align:'right'});doc.setFont('helvetica','bold');doc.text('RETRADE '+gbp(r.retrade),R,y+3,{align:'right'});
      if(adj){doc.setFont('helvetica','normal');doc.setFontSize(6.8);colour(doc.setTextColor,muted);doc.text('Account credit allocated '+gbp(Math.abs(r.accountAdjustment)),L+23,y+8);}y+=h;rule();
    });

    if(s.accountAdjustments&&s.accountAdjustments.length){y+=6;section('Account adjustments — non-cash');s.accountAdjustments.forEach(function(a){row(a.date+' · '+a.reason+(a.itemAllocations&&a.itemAllocations.length?' · '+a.itemAllocations.length+' linked items':''),'−'+gbp(a.amount),true);});}
    ...
    s.payments.forEach(function(p){
      ...
      if(p.accountAdjustmentAmount){ensure(7);doc.setFontSize(7.2);doc.setFont('helvetica','normal');colour(doc.setTextColor,muted);doc.text('Gross liabilities '+gbp(p.grossAmount)+' · account credit −'+gbp(p.accountAdjustmentAmount)+' · cash paid '+gbp(p.amount),L+5,y);y+=7;}
      p.allocations.forEach(function(a){ensure(7);doc.setFontSize(7.1);colour(doc.setTextColor,navy);doc.text(doc.splitTextToSize('↳ '+a.item+(a.saleNo?' · '+a.saleNo:''),138),L+5,y);doc.text(gbp(a.amount),R-2,y,{align:'right'});y+=7;});
    });
    ...
    if(theme&&typeof theme.addFooter==='function')theme.addFooter(doc,'Partner Statement · '+String(s.account.name||'Partner')+' · '+s.period.label);
      ...
      var pages=doc.getNumberOfPages();for(var i=1;i<=pages;i++){doc.setPage(i);doc.setFontSize(7);colour(doc.setTextColor,muted);colour(doc.setDrawColor,line);doc.line(L,285,R,285);doc.text('RETRADE · '+String(s.account.name||'Partner')+' · '+s.period.label,L,290);doc.text('Page '+i+' of '+pages,R,290,{align:'right'});}
    }
  ...
  console.info('[RETRADE] adjustment-aware partner statement exporters v1.4.99 loaded');
})();