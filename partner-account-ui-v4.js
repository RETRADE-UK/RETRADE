/* RETRADE partner account UI v1.4.80
 * Normalises every Partner account page to one finance-first layout.
 *
 * - Back + Statement stay in the top navigation row.
 * - Legacy page-wide Allocate & settle is retired.
 * - Account Position always remains four FINANCIAL cards.
 * - Potential remaining is refreshed from live unsold item data.
 * - Paid to partner is refreshed from settlement history.
 * - Legacy supplier stock-count panels are hidden outside real stock groups.
 */
(function(){
  'use strict';
  if(window.__rtPartnerAccountV4_1480)return;
  window.__rtPartnerAccountV4_1480=true;
  ...
  function findBack(page){
    ...
    for(var i=0;i<controls.length;i++){
      var raw=text(controls[i]).toLowerCase(),clean=normaliseBackLabel(raw);
  ...
  function activePage(){
    ...
    return p&&p.classList.contains('on')&&(p.querySelector('.account-group')||p.querySelector('.rt-partner-summary-v3')||findBack(p))?p:null;
  }
  ...
  function accountFromPage(page){
    ...
    if(link){
      try{
        var month=link.getAttribute('data-month'),itemId=link.getAttribute('data-itemid'),rows=(DB&&Array.isArray(DB[month]))?DB[month]:[];
        var item=rows.find(function(x){return x&&String(x.id)===String(itemId);});
        if(item&&item.accountId!=null){var aa=accountById(item.accountId);if(aa)return aa;}
      }catch(_){}
    }
    var h=page.querySelector('.page-title,h1,h2,h3'),name=text(h).toLowerCase();
    if(name){
      try{var m=(_accounts||[]).filter(function(a){var n=String(a&&a.name||'').trim().toLowerCase();return n&&(name===n||name.indexOf(n)!==-1||n.indexOf(name)!==-1);});if(m.length===1)return m[0];}catch(_){}
    }
    return null;
  }
  function accountItems(id){
    var out=[];
    try{(typeof allDBKeys==='function'?allDBKeys():[]).forEach(function(month){((DB&&DB[month])||[]).forEach(function(i){if(i&&String(i.accountId)===String(id))out.push(i);});});}catch(_){}
    return out;
  }
  ...
  function installStyles(){
    ...
    s.textContent='.rt-partner-v4-navrow{display:flex!important;align-items:center!important;justify-content:space-between!important;gap:12px!important;width:100%!important;box-sizing:border-box!important;margin:0 0 12px!important}.rt-partner-v4-navrow>.rt-partner-statement-btn{margin:0 0 0 auto!important;flex:0 0 auto!important}.rt-partner-summary-v3.rt-partner-summary-v4{margin:12px 0 16px!important}.rt-partner-summary-v4 .rt-partner-summary-v3-grid{grid-template-columns:repeat(4,minmax(0,1fr))!important}.rt-partner-v4-hidden{display:none!important}@media(max-width:760px){.rt-partner-summary-v4 .rt-partner-summary-v3-grid{grid-template-columns:repeat(2,minmax(0,1fr))!important;gap:8px!important}}@media(max-width:420px){.rt-partner-summary-v4 .rt-partner-summary-v3-grid{grid-template-columns:1fr!important}}';
  ...
  function removeLegacyAllocate(page){
    page.querySelectorAll('button,a').forEach(function(el){
      ...
      var label=text(el).toLowerCase(),action=String(el.getAttribute('onclick')||'').toLowerCase();
  ...
  function ensureDefaultCollapsedGroups(page){
    ...
    page.querySelectorAll('.account-group-head').forEach(function(head){
      var m=String(head.getAttribute('onclick')||'').match(/_toggleAccountGroup\('([^']*)','([^']*)'/);if(!m||!allowed[m[2]])return;
      var key=String(m[1])+':'+String(m[2]),collapse=false;
      try{if(typeof _accountGroupState!=='undefined'&&_accountGroupState&&!Object.prototype.hasOwnProperty.call(_accountGroupState,key)){_accountGroupState[key]=true;collapse=true;}}catch(_){collapse=true;}
      if(collapse){var g=head.closest('.account-group');if(g)g.classList.add('collapsed');head.setAttribute('aria-expanded','false');}
    });
  }
  ...
  function potentialInfo(acct){
    ...
    accountItems(acct.id).forEach(function(item){
      ...
      if(type!=='supplier'){
        var partner=0;if(fixed!=null)partner=fixed;else{var pct=item.accountSplitPercent!=null?Number(item.accountSplitPercent):Number(acct.defaultSplitPercent);if(isFinite(pct))partner=Math.max(0,pool)*Math.max(0,Math.min(100,pct))/100;}retrade=pool-partner;
      }
  ...
  function findCard(summary,label,kind){
    var cards=summary.querySelectorAll('.rt-partner-summary-v3-card'),want=String(label||'').toLowerCase();
    for(var i=0;i<cards.length;i++){var l=text(cards[i].querySelector('.rt-partner-summary-v3-label')).toLowerCase();if((kind&&cards[i].getAttribute('data-kind')===kind)||l===want)return cards[i];}return null;
  }
  function ensurePaidCard(summary,acct){
    var grid=summary.querySelector('.rt-partner-summary-v3-grid');if(!grid)return null;
    var card=findCard(summary,'Paid to partner','paid');
  ...
  function normaliseSummary(page,acct){
    ...
    // Remove only legacy stock presentation; never repurpose a finance card.
    ...
    var paid=paidInfo(acct),out=findCard(summary,'Partner outstanding','outstanding');if(out&&paid.value>0){var sub=out.querySelector('.rt-partner-summary-v3-sub'),base=text(sub).replace(/\s*·\s*£[\d,.]+\s+paid$/i,'');setText(sub,base+' · '+money(paid.value)+' paid');}
    ensurePaidCard(summary,acct);
  ...
  function fix(){queued=false;installStyles();var page=activePage();if(!page)return;removeLegacyAllocate(page);ensureTopNav(page);ensureDefaultCollapsedGroups(page);var acct=accountFromPage(page);if(acct)normaliseSummary(page,acct);hideLegacyStock(page);}
  function schedule(){if(queued)return;queued=true;requestAnimationFrame(function(){requestAnimationFrame(fix);});}
  function start(){schedule();if(observer)return;observer=new MutationObserver(schedule);observer.observe(document.body,{childList:true,subtree:true});window.addEventListener('hashchange',schedule);window.addEventListener('popstate',schedule);}
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',start,{once:true});else start();
  console.info('[RETRADE] v1.4.80 finance-first partner account normaliser loaded');
})();