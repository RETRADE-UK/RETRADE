/* RETRADE partner payment allocations v2 — v1.4.77
 *
 ...
 * - New/reconstructed payments use the canonical settlement transaction shape,
 *   so payment history, statements and cashflow stay reconciled.
 * - No automatic data migration: history changes only after an explicit action.
 */
(function(){
  'use strict';
  ...
  function paymentHistory(page){
    var groups=Array.prototype.slice.call(page.querySelectorAll('.account-group'));
    for(var i=0;i<groups.length;i++){var t=String((groups[i].querySelector('.account-group-title')||{}).textContent||'').trim().toLowerCase();if(/(settlement|payment).*(history)|history.*(settlement|payment)/.test(t))return groups[i];}
    var cards=Array.prototype.slice.call(page.querySelectorAll('.panel-card,.card,section'));
    for(var j=0;j<cards.length;j++){var h=cards[j].querySelector('.sl,.section-title,h2,h3,h4'),x=String(h&&h.textContent||'').trim().toLowerCase();if(/(settlement|payment).*(history)|history.*(settlement|payment)/.test(x))return cards[j];}
    return null;
  }
  ...
  function render(a){
    var page=document.getElementById('p-item');if(!page||!page.classList.contains('on')||!a)return;
    activeAccountId=a.id;css();
    var old=page.querySelector('.rt-payalloc2');if(old)old.remove();
    ...
    function refresh(){var ps=picked(a),sum=pickedTotal(a),copy=box.querySelector('.rt-payalloc2-summary');copy.innerHTML=ps.length?'<strong>'+ps.length+' selected · '+esc(money(sum))+'</strong>One transaction will hold these item allocations.':'<strong>'+rows.length+' items · '+esc(money(total))+'</strong>'+(rows.length?'Select items that were paid together.':'Nothing needs assigning.');var dis=!ps.length;box.querySelector('.payalloc-existing').disabled=dis;box.querySelector('.payalloc-new').disabled=dis;box.querySelector('.payalloc-all').disabled=!rows.length;}
  ...
  function openNew(a){
    var rows=picked(a);if(!rows.length)return;var legacy=rows.filter(function(r){return r.legacy;}),unpaid=rows.filter(function(r){return r.unpaid;});
    if(legacy.length&&unpaid.length){try{toast('Select legacy settled items or unpaid items separately','error');}catch(_){}return;}
    var total=pickedTotal(a),reconstruct=legacy.length>0;draft={accountId:a.id,ids:rows.map(function(r){return r.item.id;}),mode:'new',reconstruct:reconstruct};
    ...
    var intro=reconstruct?'This rebuilds one historical payment from items the old system marked settled but never linked to a transaction. The dated payment will also restore the matching historical cashflow outflow.':'This records one payment transaction and one cashflow outflow for the selected items.';
    var body='<div style="font-size:12px;color:var(--text-secondary);margin-bottom:12px;">'+esc(intro)+'</div><div class="panel-card" style="margin-bottom:14px;">'+list+'<div class="rt-payalloc2-panel-row"><span><strong>Total payment</strong></span><strong>'+esc(money(total))+'</strong></div></div><div class="fg"><label>Payment date</label><input type="date" id="payalloc-date" value="'+today()+'"></div><div class="fg"><label>Note / bank reference <span style="font-weight:400">optional</span></label><input type="text" id="payalloc-note" placeholder="e.g. Bank transfer ref"></div><button type="button" class="btn btn-primary" style="width:100%" onclick="_rtPayAllocCreate()">'+(reconstruct?'Reconstruct':'Record')+' payment · '+esc(money(total))+'</button>';
    try{openPanel((reconstruct?'Reconstruct':'New')+' payment transaction',body);}catch(_){}
  }
  ...
  function finish(a,msg){try{saveDB();}catch(e){console.warn('[RETRADE] payment allocation save failed',e);try{toast('Could not save payment transaction','error');}catch(_){}return;}selected[String(a.id)]=new Set();draft=null;try{closePanel();}catch(_){}try{toast(msg);}catch(_){}try{_renderAccountPage(a);}catch(_){} }

  window._rtPayAllocAssignExisting=function(txId){var d=liveDraft(),a=d.a,rows=d.rows;if(!a||!rows.length)return;var tx=(a.settlements||[]).find(function(x){return x&&String(x.id)===String(txId);});if(!tx)return;var total=round(rows.reduce(function(s,r){return s+r.remaining;},0)),free=Math.max(0,round((Number(tx.partnerAmount)||0)-txUsed(tx)));if(free<total-0.009){try{toast('That payment no longer has enough unallocated balance','error');}catch(_){}return;}if(!Array.isArray(tx.items))tx.items=[];rows.forEach(function(r){tx.items.push({id:r.item.id,itemId:r.item.id,name:r.item.item||'',amount:round(r.remaining)});markIfFullyPaid(a,r.item);});tx.updatedAt=new Date().toISOString();tx.allocationRepairedAt=tx.updatedAt;finish(a,'Assigned '+rows.length+' item'+(rows.length===1?'':'s')+' to '+money(tx.partnerAmount||0)+' payment');};

  window._rtPayAllocCreate=function(){
    ...
    var tx={id:'stl_'+Date.now()+'_'+Math.random().toString(36).slice(2,8),date:date,createdAt:new Date().toISOString(),paid:true,kind:fixed?'supplier':String(a.accountType||'consignment'),partnerAmount:total,yourAmount:fixed?0:round(gross-total),grossProfit:fixed?null:gross,note:note||null,items:rows.map(function(r){return {id:r.item.id,itemId:r.item.id,name:r.item.item||'',amount:round(r.remaining)};}),arrangementModel:model(a),historicalReconstruction:!!draft.reconstruct};
    if(!Array.isArray(a.settlements))a.settlements=[];a.settlements.unshift(tx);rows.forEach(function(r){markIfFullyPaid(a,r.item);});finish(a,(draft.reconstruct?'Reconstructed ':'Paid ')+money(total)+' · '+rows.length+' item'+(rows.length===1?'':'s'));
  };
  ...
  console.info('[RETRADE] partner payment allocation workflow v2 v1.4.77 loaded');
})();