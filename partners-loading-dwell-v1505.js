/* RETRADE Partners minimum loading dwell — v1.5.05
...
(function(){
  'use strict';
  if(window.__rtPartnersLoadingDwell1505)return;
  window.__rtPartnersLoadingDwell1505=true;

  var MIN_MS=240, MAX_MS=1600, token=0;
  ...
  function styles(){
    if(document.getElementById('rt-partners-dwell1505-style'))return;
    var s=document.createElement('style');s.id='rt-partners-dwell1505-style';s.textContent='\
...
@media(max-width:760px){.rt-pd1505-strip,.rt-pd1505-row{grid-template-columns:minmax(0,1fr) minmax(108px,auto) 14px;gap:12px}.rt-pd1505-total{width:78px}}@media(max-width:560px){.rt-pd1505-head{margin-bottom:18px}.rt-pd1505-title{font-size:29px}.rt-pd1505-sub{font-size:11px;line-height:1.35}.rt-pd1505-add{width:96px;height:40px}.rt-pd1505-row{min-height:82px;padding:13px}.rt-pd1505-search{height:46px}}@media(prefers-reduced-motion:reduce){.rt-pd1505-shimmer:after{animation:none!important}}';document.head.appendChild(s);
  ...
  function show(page){if(!page)return 0;var my=++token,start=Date.now();page.classList.add('rt-partners-dwell1505');shell(page);function tick(){if(my!==token)return;if(!page.classList.contains('on')){cleanup();return;}var elapsed=Date.now()-start;if(elapsed>=MIN_MS&&(ready(page)||elapsed>=MAX_MS)){requestAnimationFrame(cleanup);return;}requestAnimationFrame(tick);}function cleanup(){if(my!==token)return;var s=page.querySelector('.rt-partners-dwell-shell1505');if(s)s.remove();page.classList.remove('rt-partners-dwell1505');}requestAnimationFrame(tick);return my;}

  function install(){
    styles();var current=window.renderAccountsPage;if(typeof current==='function'&&!current.__rtDwell1505){function wrapped(){var page=document.getElementById('p-accounts');if(page)show(page);var r=current.apply(this,arguments);page=document.getElementById('p-accounts');if(page&&!page.querySelector('.rt-partners-dwell-shell1505'))show(page);return r;}wrapped.__rtDwell1505=true;wrapped.__rtBase=current;window.renderAccountsPage=wrapped;try{renderAccountsPage=wrapped;}catch(_){} }
    var page=document.getElementById('p-accounts');if(page&&page.classList.contains('on'))show(page);
  }
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',install,{once:true});else install();
  console.info('[RETRADE] v1.5.05 Partners minimum skeleton dwell loaded');
})();