...
(async()=>{
 ...
 try{
  ...
  for(const width of [1440,1024,768,390,320]){
   ...
   await page.locator(width>=1281?'#tax-explain-adjustments summary':'#tax-reconciliation summary').click();
  ...
  const expected=await page.evaluate(()=>JSON.stringify(_taxExportData));
  await page.locator('#tax-explain-adjustments summary').click();
  assert(await page.locator('.tax-desktop-reconciliation small').first().isVisible(),'Explanations reveal adjustment notes');
  await page.locator('#tax-explain-adjustments summary').click();
  assert.equal(await page.evaluate(()=>JSON.stringify(_taxExportData)),expected,'Presentation does not recalculate exports');
  await page.locator('#tax-estimate-settings>summary').click();
  await page.locator('#tax-other-income').fill('42000');
  await page.locator('#tax-other-income').dispatchEvent('change');
  assert.equal(await page.evaluate(()=>_taxExportData.otherIncome),42000);
  assert(await page.locator('#tax-estimate-settings').evaluate(e=>e.open),'Editing preserves the desktop editor');
  assert(await page.locator('#tax-other-income').evaluate(e=>document.activeElement===e),'Editing preserves focus');
  await page.locator('#tax-other-income').fill('-1');
  await page.locator('#tax-other-income').dispatchEvent('change');
  assert.equal(await page.locator('#tax-other-income').inputValue(),'0','Invalid income is normalised in the visible editor');
  await page.locator('#tax-region').selectOption('scotland');
  assert.equal(await page.evaluate(()=>_taxExportData.taxRegion),'scotland');
